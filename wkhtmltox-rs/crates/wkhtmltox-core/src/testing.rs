...
impl Renderer for MockRenderer {
    ...
    fn wait_ready(&mut self, _p: PageHandle, _r: &ReadyPolicy) -> Result<()> {
        Ok(())
    }
    fn eval_json(&mut self, _p: PageHandle, _s: &str) -> Result<serde_json::Value> {
        Ok(self.probe.clone())
    }
    fn print_pdf(&mut self, _p: PageHandle, _g: &PageGeometry) -> Result<Vec<u8>> {
        Ok(self.pdf.clone())
    }
